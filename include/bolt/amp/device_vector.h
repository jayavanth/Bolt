...
{
public:
    concurrency::array<T> m_arrayContainer;
};
...
public:
    ...
    //typedef typename std::conditional<(std::is_same< array_type, ctype >::value), array_type &, arrayview_type>::type container_type;
    ...
    */
    ...
    public:
        ...
        //Container operator[]() const restrict(cpu,amp)
        //{
        //    return m_Container[m_Index];
        //}
private:
    ...
    */
    device_vector( control& ctl = control::getDefault( ) )
        : m_Size( 0 ), m_devMemory( create_empty_array_view<value_type>::getav() )
    { }
    ...
    */
    device_vector( size_type newSize, const value_type& initValue = value_type( ), bool init = true,
        ...
        : m_Size( int(newSize) ), m_devMemory( create_empty_array_view<value_type>::getav() )
    {
    ...
    */
/* redundant now since array = array_view
...
*/
    ...
    */
    ...
    template< typename Container >
    device_vector( Container& cont, control& ctl = control::getDefault( ) )
        : m_Size( int(cont.size( )) ), m_devMemory( create_empty_array_view<value_type>::getav() )
    {
		...
		if( m_Size > 0 )
        {
			...
			concurrency::extent<1> ext( static_cast< int >( m_Size ) );
			m_devMemory = container_type( ext, cont );
		}
		else
        {
           // m_devMemory = NULL;
  ...
  //      {
		//	concurrency::extent<1> ext( static_cast< int >( m_Size ) );
		//	m_devMemory = container_type( ext, &begin[0] );
		//}
		//else
		//{
		//	 //m_devMemory = NULL;
  ...
  //  };
    ...
    */
    ...
    {
        m_Size =  int(std::distance( begin, end )); //FIXTHIS: proper conversion
    ...
    */
    ...
    ~device_vector()
    {
        //if (am_devMemory != NULL) // FIXTHIS Solve using c* mdv = m_devMemory ; bool empty = mdv;
        //{
        //    delete( m_devMemory );
        //}
    }
    ...
    */
    ...
    {
        //FIXTHIS support this after finishing conditional array reference
  ...
  //                     "This member function is only valid for concurrency::array types.");
    ...
    */
    value_type& operator[]( int ix ) const restrict(cpu,amp) //FIXTHIS change types
    {
        //const_reference tmpRef( *this, n );

        //return tmpRef;
        return m_devMemory[ix]; // change to *(*this +ix)
    }
    ...
    */
  //  reference front( void ) //FIXTHIS: implement dereference
  //  {
		//return (*begin());
  //  }
  ...
  //  *   \return Returns a const_reference, which is not a proxy object.
  //  */
  //  const_reference front( void ) const //FIXTHIS: implement dereference
  //  {
  //      return (*begin());
  //  }
  ...
  //  *   \note This returns a proxy object, to control when device memory gets mapped.
  //  */
  //  reference back( void ) //FIXTHIS: implement dereference
  //  {
		//return ( *(end() - 1) );
  //  }
  ...
  //  *   \return Returns a const_reference, which is not a proxy object.
  //  */
  //  const_reference back( void ) const //FIXTHIS: implement dereference
  //  {
		//return ( *(end() - 1) );
  //  }
    ...
    */
    void assign( size_type newSize, const value_type& value )
    {
        ...
        m_Size = int(newSize); //FIXTHIS

        arrayview_type m_devMemoryAV( m_devMemory ); //FIXTHIS
        Concurrency::parallel_for_each( m_devMemoryAV.extent, [=](Concurrency::index<1> idx) restrict(amp)
        {
    ...
    */
    ...
    {
        size_type l_Count = int(std::distance( begin, end ));
